...
{
	_appId = std::make_unique<TW_IDENTITY>();
	FillAppId();
}
TwainSession::~TwainSession()
{
...
std::unique_ptr<TW_STATUS> TwainSession::GetDsmStatus()
{
	auto status = std::make_unique<TW_STATUS>();
	if (_state > 1)
	{
		_dsmEntry(_appId.get(), nullptr, DG_CONTROL, DAT_STATUS, MSG_GET, status.get());
	}
	return status;
}
std::unique_ptr<TW_STATUS> TwainSession::GetSourceStatus()
{
	auto status = std::make_unique<TW_STATUS>();
	if (_state > 3)
	{
		DsmEntry(DG_CONTROL, DAT_STATUS, MSG_GET, status.get());
	}
	return status;
}

std::shared_ptr<TW_IDENTITY> TwainSession::ShowSourceSelector()
{
	auto src = std::make_shared<TW_IDENTITY>();
	if (_state > 2 && _state <= 7)
	{
		auto twRC = _dsmEntry(_appId.get(), nullptr, DG_CONTROL, DAT_IDENTITY, MSG_USERSELECT, src.get());
	}
	return src;
}
TW_UINT16 TwainSession::OpenSource(std::shared_ptr<TW_IDENTITY> srcId)
{
	...
	if (_state > 2 && _state < 5)
	{
		...
		twRC = _dsmEntry(_appId.get(), nullptr, DG_CONTROL, DAT_IDENTITY, MSG_OPENDS, srcId.get());
		if (twRC == TWRC_SUCCESS)
		{
			_state = 4;
			_srcId = srcId;
...
TW_UINT16 TwainSession::CloseSource()
{
	...
	if (_state == 4)
	{
		twRC = _dsmEntry(_appId.get(), nullptr, DG_CONTROL, DAT_IDENTITY, MSG_CLOSEDS, _srcId.get());
		if (twRC == TWRC_SUCCESS)
		{
			_state = 3;
			_srcId = nullptr;
...
TW_UINT16 TwainSession::DsmEntry(TW_UINT32 DG, TW_UINT16 DAT, TW_UINT16 MSG, TW_MEMREF pData)
{
	if (_dsmEntry)
	{
		return _dsmEntry(_appId.get(), _srcId.get(), DG, DAT, MSG, pData);
	}
	return TWRC_FAILURE;
}